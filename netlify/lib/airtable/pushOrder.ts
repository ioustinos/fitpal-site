...
async function reconcileDeletes(
  ...
  try {
    ...
    // records' link fields. Gather all linked rec-ids from those children
    // (order/customer links are harmless — they simply don't exist in the Order
    // Items table), then look them up by RECORD_ID(). Verified against the live
    // base (WEC-697: remove 1 dish → 0d/1i); the earlier ARRAYJOIN formula
    // matched nothing because the item→child link doesn't display the key.
